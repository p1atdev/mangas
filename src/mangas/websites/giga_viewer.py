...
class GigaViewer(WebsiteMixin):
    ...
    def parse_atom(self, pathname: str):
        atom = self.atom_parser(
            auth=self.auth,
        ).parse(url=self.url.compose(pathname))
        return atom

    def parse_root_atom(self):
        atom = self.atom_parser(
            auth=self.auth,
        ).parse(url=self.url.compose("atom"))
        return atom

    def parse_series_atom(self, series_id: str):
        atom = self.atom_parser(
            auth=self.auth,
        ).parse(url=self.url.compose("atom", "series", series_id))
        return atom

    def parse_episode_json(self, episode_id: str):
        return self.episode_parser(
            auth=self.auth,
        ).parse(url=self.url.compose("episode", episode_id + ".json"))
